            ...
            int rule_arr_size)
{
    ...
    input_mask_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, ip_arr_size * sizeof(uint64_t), input_mask, &err);
    print_err(err);
    ...
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 1, sizeof(cl_mem), &input_mask_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 2, sizeof(cl_mem), &input_sport_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 3, sizeof(cl_mem), &input_dport_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 4, sizeof(cl_mem), &input_protocol_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 5, sizeof(cl_mem), &rule_ip_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 6, sizeof(cl_mem), &rule_mask_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 7, sizeof(cl_mem), &rule_sport_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 8, sizeof(cl_mem), &rule_dport_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 9, sizeof(cl_mem), &rule_protocol_buffer);
    print_err(err);
    err |= clSetKernelArg(kernel_compare, 10, sizeof(cl_mem), &output_buffer);
    print_err(err);
    ...
    clReleaseMemObject(input_mask_buffer);
...
int main()
{
    ...
    uint64_t input_mask[ip_array_size]; // input input_mask (uint64)
...
//    freeRules(ruleList);
    ...
    for (int i = 0; i < rule_array_size; i++)
    {
        ...
        mergeBuff[0] = sMask[i];
        mergeBuff[1] = dMask[i];
        memcpy(&input_mask[i], mergeBuff, 8);
    ...
    for (int i = 0; i < rule_array_size; i++)
    {
        printf("%s %d: %u.%u.%u.%u input_mask : %u.%u.%u.%u : verdict : %d\n", "rule_ip", i, printable_ip(rule_ip[i]), printable_ip(input_mask[i]), verdict[i]);
    }
    int verdict_buffer = 0;
    for (int i = 0; i < ip_array_size * rule_array_size; i++)
    {
        test = (rule_ip[i % rule_array_size] == (input_ip[i / rule_array_size] & input_mask[i % rule_array_size])) & (protocols[i % rule_array_size] == input_protocol[i / rule_array_size]) & (sPort[i % rule_array_size] == input_sport[i / rule_array_size]) & (dPort[i % rule_array_size] == input_dport[i / rule_array_size]);
        if (test)
        {
    ...
    for (int j = 0; j < 1; j++)
    {
        compare(input_ip, input_mask, input_sport, input_dport, input_protocol, rule_ip,input_mask, sPort, dPort, protocols, tempVerdict, result, ip_array_size, rule_array_size);
