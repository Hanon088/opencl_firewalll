__kernel void compare(__global ulong *input_ip, __global ushort *input_sport,
                      __global ushort *input_dport,
                      __global uchar *input_protocol, __global ulong *rule_ip,
                      __global ulong *rule_mask, __global ushort *rule_sport,
                      __global ushort *rule_dport,
                      __global uchar *rule_protocol, __global int *result) {
  /* n rule n packet */
  ...
  if (local_rule_protocol == 0) {
    local_input_protocol = 0;
  }
  if (local_rule_sport == 0) {
    local_input_sport = 0;
  }
  if (local_rule_dport == 0) {
    local_input_dport = 0;
  }

  local_output = ((local_input_ip & local_rule_mask) == local_rule_ip) &
                 (local_rule_protocol == local_input_protocol) &
                 (local_rule_sport == local_input_sport) &
                 (local_rule_dport == local_input_dport);
  result[packet_index] = local_output;
}

__kernel void sync_rule_and_verdict(__global int *set_already_compare,
